...
import {
  ...
  normalizeAgentDetailPayload,
  normalizeAgentHistoryPayload,
...
function deriveDailyUsageFromHistory(history: AgentConversation[]): TokenUsagePoint[] {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [detailResponse, historyResponse] = await Promise.all([
        fetch(`/api/ai-agents/${agentId}`, { cache: "no-store" }),
        fetch(`/api/ai-agents/${agentId}/history`, { cache: "no-store" }),
      ]);
      ...
      const nextDetail = normalizeAgentDetailPayload(detailPayload);
      const nextHistory = normalizeAgentHistoryPayload(historyPayload).slice(0, 20);
    ...
    <div className="h-full overflow-y-auto bg-[#0a0a0f] px-4 py-5 sm:px-6">
      <div className="mx-auto w-full max-w-7xl">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div>
            <Link href="/dashboard/ai-agents" className="text-xs text-[#a29bfe] hover:text-[#b9b3ff]">
              ← Back to AI agents
            </Link>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <div className="space-y-4 xl:col-span-2">
            <section className="rounded-xl border border-[#232336] bg-[#101019] p-4">
              ...
              <div className="grid gap-3 md:grid-cols-[minmax(0,280px)_1fr]">
                <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
                  {history.length === 0 ? (
                    <p className="text-sm text-gray-500">No conversation history for this agent yet.</p>
                  ) : (
            ...
            <section className="rounded-xl border border-[#232336] bg-[#101019] p-4">
              ...
              <p className="mt-1 text-xs text-gray-400">Per-day token usage for this agent.</p>
