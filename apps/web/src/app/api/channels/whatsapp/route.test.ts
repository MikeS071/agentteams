...
describe('api/channels/whatsapp', () => {
  it('returns unauthorized without session', async () => {
    const { POST } = await loadRoute()
    setSession(null)

    const res = await POST(nextReq('http://localhost:3000/api/channels/whatsapp', { method: 'POST' }))
  ...
  it('handles incoming setup call', async () => {
    const { POST } = await loadRoute()
    setSession({ tenantId: 'tenant-1' })
    mockCheckFeatureAccess.mockResolvedValue(null)
    mockValidationSuccess({ accessToken: 'token', phoneNumberId: 'pnid' })
