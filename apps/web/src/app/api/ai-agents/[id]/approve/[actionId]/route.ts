...
export async function POST(request: Request, { params }: { params: { id: string; actionId: string } }) {
  ...
  if (!agentId || !actionId) {
    return NextResponse.json({ error: "Missing agent id or action id" }, { status: 400 });
  }

  const apiBaseURL = process.env.API_URL ?? "http://localhost:8080";

  try {
    const upstream = await fetch(`${apiBaseURL}/api/ai-agents/${encodeURIComponent(agentId)}/approve/${encodeURIComponent(actionId)}`, {
      method: "POST",
      headers: {
    ...
    console.error("AI agents approve POST error", error);
