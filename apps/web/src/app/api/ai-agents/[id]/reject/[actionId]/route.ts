...
export async function POST(request: Request, { params }: { params: { id: string; actionId: string } }) {
  ...
  try {
    const upstream = await fetch(`${apiBaseURL}/api/ai-agents/${encodeURIComponent(agentId)}/reject/${encodeURIComponent(actionId)}`, {
      method: "POST",
      headers: {
    ...
    console.error("AI agents reject POST error", error);
