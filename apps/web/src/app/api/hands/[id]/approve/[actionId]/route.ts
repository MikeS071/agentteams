...
export async function POST(request: Request, { params }: { params: { id: string; actionId: string } }) {
  ...
  try {
    const upstream = await fetch(`${apiBaseURL}/api/hands/${encodeURIComponent(handId)}/approve/${encodeURIComponent(actionId)}`, {
      method: "POST",
      headers: {
    ...
    console.error("hands approve POST error", error);
