...
function getAPIURL(id: string) {
  ...
  return `${apiBaseURL}/api/hands/${id}`;
}
...
export async function GET(_: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("hand detail GET error", error);
    return NextResponse.json({ error: "Failed to load hand" }, { status: 500 });
  }
}
...
export async function PUT(req: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("hand detail PUT error", error);
